...
class LatentDiffusion(pl.LightningModule):
    ...
    def forward(self, *args, **kwargs):
        ...
        return self.output_T(
            self.ae.decode(self.model(*args, **kwargs) / self.latent_scale_factor)
        )
    ...
    def training_step(self, batch, batch_idx):
        latents = (
            self.ae.encode(self.input_T(batch)).detach() * self.latent_scale_factor
        )
...
class LatentDiffusionConditional(LatentDiffusion):
    ...
    def forward(self, condition, *args, **kwargs):
        condition_latent = (
            self.ae.encode(self.input_T(condition.to(self.device))).detach()
            * self.latent_scale_factor
        )

        output_code = (
            self.model(condition_latent, *args, **kwargs) / self.latent_scale_factor
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            latents = (
                self.ae.encode(self.input_T(output)).detach() * self.latent_scale_factor
            )
            latents_condition = (
                self.ae.encode(self.input_T(condition)).detach()
                * self.latent_scale_factor
            )
